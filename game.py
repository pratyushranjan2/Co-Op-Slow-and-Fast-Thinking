...
class Actions:
    ...
    getMASPossibleActions = staticmethod(getMASPossibleActions)
...
class Game:
    ...
    def run( self ):
        ...
        for i in range(len(self.mas_agents)):
            agent = self.mas_agents[i]
            if not agent:
                self.mute(i)
        ...
        return self.state.data.scores, self.state.data.deadPacmans, self.steps_alive, self.state.data.foodEaten, self.state.data.agentScores
